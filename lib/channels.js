...
module.exports = function (args) {
    ...
    this.buildNew = function () {
        var walk = function (i) {
            ...
            setTimeout(function () {
                ...
                add.on('ready', function () { // @TODO - exclude it from respawn - DONE!
    ...
    this.mapInputs = function(){
        if (global.config.inputs.enable != true)
            return;

        log(' CHANNELS MAPPING INPUTS');
        that.channels.forEach(function (channel) {
            if (channel.options.input_index) {
                channel.setInput(channel.options.input_index);
            }
        });
    };
