...
module.exports = function (args) {
    ...
    this.init = function () {
        ...
        that.on('address', function (mpd, chunk) {
            if (that.ready !== true) {
                log(' !!! MPD ADDRESS IN USE', that.name);
                that.emit('ready', that);
            }
        });
        that.on('data', function (chunk) {
            if (that.options.debug.stderr === true) {
                log(' MPD OUT', that.name, '\n' + chunk);
            }
        });
        that.on('no-soundcard', function (mpd, chunk) {
            log(' MPD NO SOUNDCARD FOUND', that.name);
        });
        that.on('update', function (mpd, chunk) {
            log(' MPD DB UPDATE', that.name);
        });
        that.on('added', function (mpd, chunk) {
            log(' MPD DB UPDATE', that.name, chunk.replace(new RegExp('\n', 'i'), '').replace('update: added /', ''));
        });
    ...
    this.kill = function () {
        ...
        log(' MPD KILLING', that.name);
        spawn(that.options.bin, options);
    };

    this.run = function () {
        that.kill();
        var options = [that.options.conf_file, '--no-daemon', '--stdout', '--verbose', '--stderr'];
        log(' MPD STARTING', that.name, 'WITH OPTIONS', JSON.stringify(options));
        var last_chunk = '';
        var match = {
            ready: [
                new RegExp(/successfully established/),
                new RegExp(/Client is CONNECTING/)
            ],
